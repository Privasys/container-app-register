...
func (m *Material) CommitmentKey(delivered *[32]byte) (ck [32]byte, source string, err error) {
	if delivered != nil {
		...
		derived, dErr := hkdf.Key(sha256.New, m.Master[:], nil, "register/commitment-key/v1", 32)
		if dErr != nil {
...
func (m *Material) checkCommitmentKey(ck [32]byte) error {
	...
	mac.Write([]byte("register/ck-check/v1"))
	want := mac.Sum(nil)
...
func Fingerprint(ck [32]byte) []byte {
	sum := sha256.Sum256(append([]byte("register/ck-fingerprint/v1"), ck[:]...))
	return sum[:]
}
