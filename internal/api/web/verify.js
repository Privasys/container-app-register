...
// checks the register's signature; neither step asks the register to be
...
// which stays inside the enclave; the register's signature over the
...
// canonical renders a value the way the register hashes it: object
...
async function verifyBundle(bundle, publicKeyB64) {
  ...
  // The register signs the bundle with its signature field emptied, not
  ...
  if (!anchor) {
    checks.push({ name: 'Checkpoint anchor', state: 'unknown',
      why: 'the bundle carries no checkpoint; ask the register to issue one' });
...
window.RegisterVerify = { verifyBundle, verifyProof, canonical, bytesToHex };
